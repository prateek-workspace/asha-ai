...
export const NutritionPlanner = () => {
  ...
  const nextDay = () => {
    ...
    setTabletTaken(false); // Reset for demo
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <button onClick={prevDay} className="p-3 bg-gray-50 rounded-full hover:bg-gray-100 text-gray-600 transition-colors"><ChevronLeft size={24} /></button>
        <div className="text-center">
          <h3 className="text-2xl font-bold text-gray-800">{currentDay.day}</h3>
          <p className="text-green-600 font-medium">{currentDay.localName}</p>
        </div>
        <button onClick={nextDay} className="p-3 bg-gray-50 rounded-full hover:bg-gray-100 text-gray-600 transition-colors"><ChevronRight size={24} /></button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
            ...
            >
                ...
                <div key={idx} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all flex gap-4 items-start group">
                    <div className={`h-14 w-14 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0 shadow-inner transition-transform group-hover:scale-110 ${meal.type === 'Breakfast' ? 'bg-orange-50' : 'bg-green-50'}`}>
                    {meal.type === 'Breakfast' ? '🌅' : '🍛'}
                    </div>
                    <div>
                    <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">{meal.type}</p>
                    <h4 className="font-bold text-lg text-gray-800 leading-tight">{meal.item}</h4>
                    <p className="text-xs text-green-700 mt-2 flex items-center gap-1 bg-green-50 px-2 py-1 rounded-md w-fit font-medium border border-green-100">
        ...
        <div className="space-y-6">
            ...
            {currentDay.ifaTablet ? (
                <div className={cn(
                    ...
                    tabletTaken ? "bg-green-50 border-green-200" : "bg-red-50 border-red-100"
                )}>
                    <div className={cn(
                        "h-16 w-16 rounded-full flex items-center justify-center shadow-sm transition-colors",
                        tabletTaken ? "bg-green-100 text-green-600" : "bg-white text-red-500"
                    )}>
                         {tabletTaken ? <Check size={32} /> : <span className="text-3xl">💊</span>}
                    </div>
                    <div>
                        <h4 className={cn("font-bold text-lg", tabletTaken ? "text-green-800" : "text-red-800")}>
                            {tabletTaken ? "Tablet Taken!" : "Iron (IFA) Tablet"}
                        </h4>
                        <p className={cn("text-sm mt-1", tabletTaken ? "text-green-600" : "text-red-600")}>
                    ...
                    <button 
                        ...
                        className={cn(
                            ...
                            tabletTaken ? "bg-white text-green-600 border border-green-200" : "bg-red-600 text-white hover:bg-red-700 shadow-lg shadow-red-200"
                        )}
                    >
                        {tabletTaken ? "Undo" : "Mark as Taken"}
                    </button>
                </div>
            ) : (
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                <h3 className="font-bold text-gray-800 mb-4 text-sm uppercase tracking-wider">Local Superfoods</h3>
                <div className="space-y-3">
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-yellow-50 border border-yellow-100">
                        <span className="text-2xl">🍯</span>
                        <div>
                            <p className="font-bold text-gray-800 text-sm">Jaggery (Gud)</p>
                            <p className="text-xs text-gray-500">Natural Iron Source</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-green-50 border border-green-100">
                        <span className="text-2xl">🥬</span>
                        <div>
                            <p className="font-bold text-gray-800 text-sm">Spinach (Palak)</p>
                            <p className="text-xs text-gray-500">Vitamin Rich</p>
                        </div>
                    </div>
                </div>
            </div>
