...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({ title, duration, lang }) => {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-3 flex items-center gap-3 shadow-sm">
      <button 
        onClick={togglePlay}
        className={`h-10 w-10 rounded-full flex items-center justify-center flex-shrink-0 transition-colors ${isPlaying ? 'bg-pink-600 text-white' : 'bg-pink-100 text-pink-600'}`}
      >
